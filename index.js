    ...
    .then(db => {
        ...
        app.get('/payouts/:shopId', (req, res) => {
            ...
            paymentModule.getPayouts().then(payouts => {
                    ...
                    payouts = payouts.filter(payout => payout.data.shopId == shopId).reverse();
                    res.send(payouts)
                })
                .catch(err => {
                    console.log(err)
...
const handleDonation = async (payment) => {
    ...
    const all_receivers_with_addresses = data.filter((receiver) => validAddress(receiver.donation_address));
    ...
    console.log("all_receivers_with_addresses count", all_receivers_with_addresses.length)

    //remove spent addresses
    let addresses = all_receivers_with_addresses.map(e => e.donation_address.slice(0, 81))
    let spentStatus = await wereAddressesSpentFrom(addresses)
    let receivers_with_valid_addresses = all_receivers_with_addresses.filter((obj, index) => spentStatus[index] == false)

    console.log("receivers_with_valid_addresses count", receivers_with_valid_addresses.length)
    ...
    let average_iota = Math.floor(total_iotas / receivers_with_valid_addresses.length)
    ...
    for(receiver of receivers_with_valid_addresses){
        try {
            if(receiver.iotas>0){
                ...
                console.log(`Payout with ${payout.value} created for shop (${receiver.name}). Address: ${payout.address}`);
                //wait 1 second
